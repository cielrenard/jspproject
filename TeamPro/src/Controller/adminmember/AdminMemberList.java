...
public class AdminMemberList implements BrSuperController {
	...
	public ServiceForward execute(HttpServletRequest req, HttpServletResponse resp) {
		...
		// 총데이터개수 구하기
		String sql1 = MemberSql.getSQL(MemberSql.ADMIN_MEMBER_COUNT);
		...
		String sql =MemberSql.getSQL(MemberSql.ADMIN_MEMBER_LIST);
		...
		if(list!=null) {
			req.setAttribute("adminlist", true);
		}else {
			req.setAttribute("adminlist", false);
		}
		...
		forward.setPath("/VIEW/adminmember/admin_memberlist.jsp");
